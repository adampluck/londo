...
| [Jim Eaton](https://www.jimeaton.org/events/) | WooCommerce Store API (`event-bookings` category): each event is a product with description, cover image and ticket prices. Dates and times are prose in the short description ("Time:" lines, one per day; "Dates & Times:" sessions for courses) and the year comes from the product title; a weekend is one event from the first session's start to the last's end. Runs nationally (Totnes, Stroud, Bristol), so London is required positively on the "Venue:" text; London venues are given as an area only, the address being emailed on booking |
